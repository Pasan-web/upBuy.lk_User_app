...
public class InvoiceActivity extends AppCompatActivity {
    ...
    private void setAddress(String invoiceId) {
        orderCollection.document(invoiceId).get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
            ...
            public void onSuccess(DocumentSnapshot documentSnapshot) {
                ...
                addCollection.document(invoice.getAddressDocId()).get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                    @Override
                    public void onSuccess(DocumentSnapshot documentSnapshot) {
                        Address address = documentSnapshot.toObject(Address.class);
                        userCollection.document(invoice.getUserDocId()).get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                            @Override
                            public void onSuccess(DocumentSnapshot documentSnapshot) {
                                Users users = documentSnapshot.toObject(Users.class);
                                cusName.setText(users.getFirstname()+" "+ users.getLastname());
                                houseNo.setText(address.getHouseNo()+","+address.getStreetName());
                                ...
                                orderItemCollection.whereEqualTo("invoiceDocId",invoiceId).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                                    ...
                                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                                        ...
                                        for (QueryDocumentSnapshot document : task.getResult()) {
                                            ...
                                            double qt = Double.parseDouble(item.getQty());

                                            db.collection("Product").document(item.getProductDocId()).get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                                                @Override
                                                public void onSuccess(DocumentSnapshot documentSnapshot) {
                                                    ...
                                                    pdfD.setQty(item.getQty());
                                                    pdfD.setAmount(String.valueOf(Double.parseDouble(item.getQty()) * Double.parseDouble(product.getPrice())));
                                                    pdf.add(pdfD);
                                                    ...
                                                    DataTableRow row = new DataTableRow.Builder()
                                                            ...
                                                            .value(item.getQty())
                                                            .value("Rs:"+String.valueOf(Double.parseDouble(item.getQty()) * Double.parseDouble(product.getPrice())))
                                                            .build();
                                                    rows.add(row);
    ...
    private void printPdf(String invoiceId){
        ...
        canvas.drawText("UPBUY.LK ONLINE STORE",20,20,paint);
